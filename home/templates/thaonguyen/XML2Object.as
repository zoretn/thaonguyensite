...
* Example usage:
*
*   import net.produxion.core.XML2Object;
...
class XML2Object{
	...
	private function nodesToProperties( parent:XMLNode, path:Object, name:String, position:Number ):Object {
		...
		path = (path == undefined) ?  _result :  path[name];
		...
		if( parent.hasChildNodes() ){
			...
			while( nodes.length > 0 ){
				...
				if ( node.nodeName != undefined ){
					...
					if( path[node.nodeName] != undefined ){
						if( path[node.nodeName].__proto__ == Array.prototype ){
							...
							var copyObj:Object = path[node.nodeName];
				...
				if( node.hasChildNodes() ){
					nodesToProperties( node, path, name, position );
				}
			}	
		}
		return this._result;
	...
	private function sanitizeLineBreaks( raw:String ) : String {
		if( raw.indexOf( "\r\n" ) > -1 ){
			return raw.split( "\r\n" ).join( "\n" );
		}
		return raw;
