...
// Released under ThinMambo Free Software License (see TMEdit._license.txt)
...
function PopupDiv(editor, titleText, handler, initFunction) {
	...
	TMEdit._addEvent(title, "mousedown", function(ev) {
...
PopupDiv.prototype.showAt = function(x, y) {
	...
	TMEdit._addEvents(this.doc.body, ["mousedown", "click"], PopupDiv.checkPopup);
	TMEdit._addEvents(this.editor._doc.body, ["mousedown", "click"], PopupDiv.checkPopup);
	if (is_ie && this.modal) {
	...
	window.event && TMEdit._stopEvent(window.event);
...
PopupDiv.prototype.close = function() {
	...
	PopupDiv.currentPopup = null;
	this.hideShowCovered();
	TMEdit._removeEvents(this.doc.body, ["mousedown", "click"], PopupDiv.checkPopup);
	TMEdit._removeEvents(this.editor._doc.body, ["mousedown", "click"], PopupDiv.checkPopup);
...
PopupDiv.prototype._dragStart = function (ev) {
	...
	TMEdit._addEvent(this.doc, "mousemove", PopupDiv.dragIt);
	TMEdit._addEvent(this.doc, "mouseover", TMEdit._stopEvent);
	TMEdit._addEvent(this.doc, "mouseup", PopupDiv.dragEnd);
...
PopupDiv.dragEnd = function () {
	...
	TMEdit._removeEvent(popup.doc, "mouseup", PopupDiv.dragEnd);
	TMEdit._removeEvent(popup.doc, "mouseover", TMEdit._stopEvent);
	TMEdit._removeEvent(popup.doc, "mousemove", PopupDiv.dragIt);
	popup.hideShowCovered();
};
...
PopupDiv.prototype.addButtons = function() {
	...
	for (var i = 0; i < arguments.length; ++i) {
		...
		button.innerHTML = TMEdit.I18N.buttons[btn];
		switch (btn) {
		    case "ok":
			button.onclick = function() {
				self.callHandler();
